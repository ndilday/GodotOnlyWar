...
namespace OnlyWar.Helpers.Turns
{
    ...
    /// </summary>
    public sealed class MissionTurnProcessor
    {
        ...
        public MissionTurnProcessor(MissionTurnDependencies dependencies)
        {
            ...
            _sector = dependencies.Sector;
            _rules = dependencies.Rules;
            ...
            _invasionForces = dependencies.InvasionForces ?? Array.Empty<StrategicInvasionForce>();
            _factionRules = dependencies.FactionRules;
            ...
            _recordIntelGain = dependencies.RecordIntelGain;
            _recordTargetObservation = dependencies.RecordTargetObservation;
            _recordScenarioPdfLost = dependencies.RecordScenarioPdfLost;
        }

        public void ProcessStrategicCombatMissions(
            ...
            ICollection<StrategicCombatResult> strategicCombatResults)
        {
            var resolver = new StrategicCombatResolver(
                ...
                strategicInvasionForces: _invasionForces);
            foreach (Order order in strategicCombatOrders)
            {
                ...
                if (!order.StrategicInvasionForceId.HasValue)
                {
                    List<long> inferredIds = order.AssignedSquads
                        .Select(squad => _invasionForces.FirstOrDefault(force =>
        ...
        public void ProcessCombatMissions(
            ...
            ICollection<ConstructionProgressReport> constructionReports = null)
        {
            ...
            foreach (Order order in combatOrders)
            {
                ...
                foreach (Squad squad in order.AssignedSquads)
                {
                    ...
                    else if (isPlayerOrder)
                    {
                        if (readiness?.StructuralBlockers.Count > 0)
                        {
                            ...
                            string message =
                                $"{squad.Name} withheld: {string.Join(", ", readiness.StructuralBlockers.Select(ReadinessDescriptions.BlockerLabel))}.";
                ...
                if (isPlayerOrder)
                {
                    readinessMessages.AddRange(order.AssignedCharacters
                        .Where(character => !_readiness.EvaluateSoldier(character, doctrine: doctrine, program: _recruitment).IsDutyReady)
                        .Select(character =>
            ...
            IReadOnlyList<BaseSkill> medicalSkills =
                _resolveMedicalSkills?.Invoke() ?? Array.Empty<BaseSkill>();
            ...
            ICollection<MissionContext> missionContexts)
        {
            ...
            PlanetFaction observer = region?.Planet?.PlanetFactionMap
                .GetValueOrDefault(order.OwnerFaction?.Id ?? _rules.PlayerFaction.Id);
        ...
        public static void ResolveReciprocalAssaults(
        ...
        public static bool AreReciprocalAssaults(MissionContext first, MissionContext second)
        {
        ...
        {
            _applyDailyHealing?.Invoke(
                _sector?.PlayerForce?.Army?.OrderOfBattle?.GetAllMembers());
        }
        ...
        /// </summary>
        private void ApplyDailyFieldCare(
        ...
        {
            foreach (Planet planet in _sector.Planets.Values)
            {
        ...
        public static void ProcessConstructionOrders(IEnumerable<Order> constructionOrders)
        {
        ...
        {
            BaseSkill engineering = _rules.Skills.EngineeringFortification;
        ...
        /// </remarks>
        public static void ApplyConstructionPoints(ConstructionMission mission, double points)
        {
        ...
        /// </summary>
        public static void ApplyConstruction(ConstructionMission mission, double amount)
        {
            double before = GetConstructionLevel(mission);
            if (mission.ConstructionType == DefenseType.Organization)
            {
        ...
        public static double GetSharedConstructionLevel(ConstructionMission mission) =>
        ...
        public static double GetConstructionLevel(ConstructionMission mission)
        {
        ...
        public static string DescribeRegionFaction(RegionFaction regionFaction)
        {
