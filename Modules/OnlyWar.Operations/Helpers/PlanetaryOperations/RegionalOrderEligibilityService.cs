...
namespace OnlyWar.Helpers.PlanetaryOperations
{
    ...
    public static class RegionalOrderEligibilityService
    {
        public static RegionalEligibilityResult Build(
            Sector sector,
            Region target,
            IReadinessDecisions readinessDecisions,
            ...
            Order contextOrder = null)
        {
            ...
            // Doctrine and reservations are resolved from the force this sector belongs to, not from
            // whichever campaign is installed (SB-05a).
                        ...
                        playerFaction.Id, out RegionFaction playerPresence))
                {
                        ...
                        .ThenBy(squad => squad.Id))
                    {
                        RegionalSquadCandidate candidate = Evaluate(
                            squad, origin, target, selectedMission, contextOrder, force, readinessDecisions);
            ...
            IReadinessDecisions readinessDecisions)
        {
            ...
            SquadReadinessSnapshot readiness = readinessDecisions.EvaluateSquad(
                squad,
                program: ForceReadinessInputs.ProgramFor(force, squad),
                doctrine: ForceReadinessInputs.DoctrineFor(force, squad));
