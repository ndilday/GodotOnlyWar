...
namespace OnlyWar.Runtime.WorldGeometry
{
    public static class SubsectorBuilder
    {
        ...
        {
            public PlanePoint Center;
            public float RadiusSquared;
        }

        public static List<Subsector> BuildSubsectors(IEnumerable<Planet> planets, GridCell gridDimensions, ushort maxDiameter)
        {
            Dictionary<ushort, List<Planet>> subsectorPlanetMap = [];
            Dictionary<ushort, Circle> subsectorCircleMap;
            Dictionary<ushort, List<GridCell>> subsectorCellListMap = [];
            ...
            foreach(Planet planet in planets)
            {
                ...
                GridCell gridPosition = new GridCell((int)planet.Position.X, (int)planet.Position.Y);
        ...
        private static Dictionary<ushort, List<GridCell>> AssignGridSubsectors(Dictionary<ushort, List<Planet>> subsectorPlanetMap, Dictionary<ushort, Circle> subsectorCircleMap,
                                                                               GridCell gridDimensions, ushort subsectorMaxRadius)
        {
            Dictionary<ushort, List<GridCell>> subsectorCellListMap = [];
            ...
            for (int j = 0; j < gridDimensions.Y; j++)
            {
                ...
                {
                    GridCell gridPos = new GridCell(i, j);
        ...
        private static Dictionary<ushort, Circle> CalculateSubsectorCircles(Dictionary<ushort, List<Planet>> subsectorPlanetMap)
        {
            ...
            {
                List<PlanePoint> points = subsectorPlanetList.Value.Select(p => new PlanePoint(p.Position.X, p.Position.Y)).ToList();
                circles[subsectorPlanetList.Key] = FindMinimumEnclosingCircle(points, new List<PlanePoint>());
            }
            return circles;
        }

        private static Circle FindMinimumEnclosingCircle(List<PlanePoint> points, List<PlanePoint> boundary)
        {
            ...
            PlanePoint point = points[points.Count - 1];
        ...
        private static Circle CalculateCircle(List<PlanePoint> boundary)
        {
            if (boundary.Count == 0)
            {
                return new Circle { Center = new PlanePoint(0, 0), RadiusSquared = 0 };
            }
            ...
            {
                PlanePoint newCenter = new PlanePoint((boundary[0].X + boundary[1].X) / 2, (boundary[0].Y + boundary[1].Y) / 2);
                return new Circle
                {
                    Center = newCenter,
            ...
            else
            {
                ...
                {
                    PlanePoint midpoint = new PlanePoint((boundary[0].X + boundary[1].X) / 2, (boundary[0].Y + boundary[1].Y) / 2);
                    return new Circle
                    {
                        Center = midpoint,
                ...
                {
                    PlanePoint midpoint = new PlanePoint((boundary[0].X + boundary[2].X) / 2, (boundary[0].Y + boundary[2].Y) / 2);
                    return new Circle
                    {
                        Center = midpoint,
                        RadiusSquared = CalculateDistanceSquared(boundary[0], midpoint)
                ...
                {
                    PlanePoint midpoint = new PlanePoint((boundary[1].X + boundary[2].X) / 2, (boundary[1].Y + boundary[2].Y) / 2);
                    return new Circle 
                    { 
                        Center = midpoint, 
                ...
                else
                {
                    ...
                    PlanePoint v21 = boundary[2] - boundary[1];
                    PlanePoint v02 = boundary[0] - boundary[2];
                    PlanePoint v10 = boundary[1] - boundary[0];
                    ...
                    PlanePoint newCenter = new PlanePoint(centerX, centerY);
                    return new Circle
                    {
                        Center = newCenter,
                        RadiusSquared = CalculateDistanceSquared(boundary[0], newCenter)
        ...
        private static bool IsInsideCircle(Circle circle, PlanePoint point)
        {
        ...
        private static float CalculateDistanceSquared(PlanePoint coordinate1, PlanePoint coordinate2)
        {
