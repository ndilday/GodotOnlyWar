...
internal sealed class ChapterScreenContext
{
    ...
    internal ChapterBrowserView QueryChapterBrowser(ChapterBrowserQuery query)
    {
    ...
    internal ChapterFilterOptions QueryFilterOptions(ChapterBrowserQuery query)
    {
        Unit chapter = TryGetChapter();
    ...
    internal int? FindCompanyForSquad(int squadId)
    {
        Unit chapter = TryGetChapter();
    ...
    internal bool CanNavigateToSquad(int squadId) =>
        SquadLocationNavigation.Resolve(
            TryGetChapter()?.GetAllSquads().FirstOrDefault(squad => squad.Id == squadId))
            is not null;

    internal ChapterPrompt DescribeTransfer(int soldierId, int optionIndex)
    {
        ...
        if (option.IsNewSquad
            || option.SoldierTemplate != _rules.ChapterDoctrine.DevastatorMarine)
        {
            return new ChapterPrompt(
                ChapterPromptKind.Blocked,
                "Promotion Blocked",
        ...
        BlackCarapacePlanResult plan = _promotionService
    ...
    internal ChapterTransferResult ConfirmTransfer(
        Guid sessionToken, int soldierId, int optionIndex, IReadOnlyList<int> contextSoldierIds)
    {
        ...
        if (SoldierTransferService.RequiresBlackCarapace(soldier, option))
        {
            RecruitmentPromotionResult promotion = _promotionService
        ...
        bool didTransfer = _transferService.ApplyTransfer(
            soldier, option, force.Army.SquadMap, _currentDate, _identity);
        if (!didTransfer)
        {
            return new ChapterTransferResult(false, ChapterPrompt.None, false, false, null, null);
        }
    ...
    internal ChapterPrompt DescribeRecall(int soldierId)
    {
        if (FindChapterSoldier(soldierId) is not PlayerSoldier soldier
    ...
    internal ChapterTransferResult ConfirmRecall(Guid sessionToken, int soldierId)
    {
        if (_sector == null
    ...
    internal static ChapterBrowserView BuildNoChapterView() => new(
        false,
    ...
    private ISoldier FindChapterSoldier(int soldierId)
    {
        ...
        return chapter?.GetAllMembers().FirstOrDefault(soldier => soldier.Id == soldierId)
            ?? _sector.PlayerForce?.Army?.FallenBrothers
    ...
    private SoldierTransferOption ResolveTransferOption(PlayerSoldier soldier, int optionIndex)
    {
        List<SoldierTransferOption> options = _transferService.GetTransferOptions(
            _sector.PlayerForce.Army.OrderOfBattle, soldier);
    ...
    private ChapterBrowserDetail BuildSquadDetail(Squad squad, ISoldier selectedSoldier)
    {
        ...
        if (selectedSoldier != null)
        {
            cards.Insert(0, new ChapterBrowserDetailCard(
                ...
                DutyStatus(_sector.PlayerForce, selectedSoldier),
