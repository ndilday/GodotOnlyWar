...
internal sealed class LoadoutScreenContext
{
    ...
    internal SquadLoadoutView QuerySquadLoadout(int squadId)
    {
    ...
    internal LoadoutCommandResult SetSquadLoadout(
    ...
    internal LoadoutCommandResult ReturnSquadToDoctrine(Guid sessionToken, int squadId)
    {
        if (RejectLoadoutCommand(sessionToken) is LoadoutCommandResult rejection) return rejection;
        Squad squad = FindPlayerSquad(squadId);
        if (squad == null) return LoadoutCommandResult.Failed("That squad is no longer available.");
    ...
    internal LoadoutCommandResult SetSquadCharacterWeaponSet(
        Guid sessionToken, int squadId, int soldierId, WeaponSet weaponSet)
    {
        ...
        CharacterLoadoutService.SetPersonalLoadout(
            soldier, weaponSet, _sector.PlayerForce);
        return LoadoutCommandResult.Ok();
    }

    internal LoadoutCommandResult ResetSquadCharacterLoadout(
        Guid sessionToken, int squadId, int soldierId)
    {
        ...
        EquipmentRulesCatalog catalog = _rules?.EquipmentCatalog;
    ...
    internal EquipmentEditorView QuerySoldierEquipmentEditor(int squadId, int soldierId)
    {
        ISoldier soldier = FindSquadMember(squadId, soldierId);
        Squad squad = FindPlayerSquad(squadId);
        SquadTemplateElement element = FindPersonalElement(soldier);
        ...
        EquipmentLoadoutDoctrine doctrine = squad.Faction?.IsPlayerFaction == true
            ? _sector.PlayerForce?.Army?.EquipmentLoadoutDoctrine
            : null;
    ...
    internal LoadoutCommandResult SaveSoldierEquipment(
    ...
    internal LoadoutDoctrineScopeView QueryDoctrineScope(int? planetId)
    {
        PlayerForce force = _sector?.PlayerForce;
    ...
    internal LoadoutTemplateDetailView QueryTemplateLoadout(int? planetId, int templateId)
    {
        PlayerForce force = _sector.PlayerForce;
    ...
    internal LoadoutCommandResult SaveTemplateLoadout(
    ...
    internal LoadoutCommandResult InheritTemplateLoadout(
    ...
    // equipped in another.
    internal IReadOnlyList<CharacterLoadoutRowData> QueryCharacterRoles()
    {
        PlayerForce force = _sector.PlayerForce;
    ...
    internal LoadoutCommandResult SetCharacterRoleWeaponSet(
    ...
    internal LoadoutCommandResult ResetCharacterRole(Guid sessionToken, int roleId)
    {
        if (RejectLoadoutCommand(sessionToken) is LoadoutCommandResult rejection) return rejection;
        PlayerForce force = _sector.PlayerForce;
        EquipmentRulesCatalog catalog = _rules?.EquipmentCatalog;
    ...
    internal EquipmentEditorView QueryRoleEquipmentEditor(int roleId)
    {
        PlayerForce force = _sector.PlayerForce;
        EquipmentRulesCatalog catalog = _rules?.EquipmentCatalog;
    ...
    internal LoadoutCommandResult SaveRoleEquipment(
    ...
    internal OperationalDoctrineView QueryOperationalDoctrine()
    {
    ...
    internal static OperationalDoctrineView EmptyOperationalDoctrine() =>
        new(InjuryThresholdLabels, 0, false, 0);

    internal string DescribeOperationalDoctrineConsequence(
    ...
    internal LoadoutCommandResult SaveOperationalDoctrine(
        Guid sessionToken,
    ...
    {
        if (_sector == null) return LoadoutCommandResult.Failed(NoCampaignMessage);
        return null;
    }
    ...
    private Squad FindPlayerSquad(int squadId) =>
        _sector.PlayerForce?.Army?.OrderOfBattle?.GetAllSquads()
    ...
    private string DescribeCharacterRow(ISoldier soldier, EffectiveCharacterLoadout resolved)
    {
        ...
        IReadOnlyList<string> honors = _dossierService.BuildCombatHonorNames(
            playerSoldier, _rules?.AwardCatalog);
