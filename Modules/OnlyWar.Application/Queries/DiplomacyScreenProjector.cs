...
namespace OnlyWar.Application
{
    ...
    public sealed class DiplomacyScreenProjector
    {
        ...
        {
            if (sector?.PlayerForce == null || rules == null) return [];
            ...
            List<TreeNode> nodes = activeRequests.Count == 0
                ? [new TreeNode(
                    0, "No outstanding requests from the sector's governors.", [], selectable: false)]
                : activeRequests.Select(request => CreateRequestNode(request, rules)).ToList();
            ...
            if (pledges.Count > 0)
            {
                nodes.Add(new TreeNode(
                    0,
                    "Outstanding pledges",
                    pledges.Select(pledge => CreatePledgeNode(sector, pledge)).ToList(),
                    selectable: false));
            }
        ...
        /// a plain-sandbox sector, or a scenario that has already resolved (a settled promise
        /// belongs to the Chapter's history, not to the board of outstanding business).
        /// </summary>
        private static TreeNode CreatePromisedWorldNode(Sector sector)
        {
            ...
            // The obligation follows the seat, not the person, so this resolves whoever holds it
            // now rather than the character who originally made the promise.
        ...
        /// <summary>
        /// How far the liberation has actually got, measured the same way the objective resolves
        /// it (ScenarioTurnProcessor): by which regions an enemy still openly holds.
        /// </summary>
        private static List<TreeNode> DescribeLiberationProgress(Planet promised)
        {
            ...
            {
                return [new TreeNode(
                    0, "Progress: unknown — no report from the world", [], selectable: false)];
            }
            ...
            int contestedRegions = enemyHoldings.Select(holding => holding.Region).Distinct().Count();
            ...
            List<TreeNode> byFaction = enemyHoldings
                .GroupBy(holding => holding.PlanetFaction.Faction.Name)
        ...
        private static TreeNode CreateRequestNode(IRequest request, GameRulesData rules)
        {
            ...
            string status = request.IsRequestStarted(rules)
            ...
            return new TreeNode(
                request.Id, $"{requesterName}, Governor of {planetName}", details, selectable: true);
        }
        ...
        private static TreeNode CreatePledgeNode(Sector sector, Pledge pledge)
        {
            string source = sector.Planets.TryGetValue(pledge.SourcePlanetId, out Planet planet)
                ? planet.Name
                : "Unknown world";
