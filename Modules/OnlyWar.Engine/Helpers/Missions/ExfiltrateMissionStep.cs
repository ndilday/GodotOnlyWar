...
namespace OnlyWar.Helpers.Missions
{
    public class ExfiltrateMissionStep : IMissionStep
    {
        ...
        public MissionStepResult ExecuteMissionStep(MissionExecutionContext execution, float marginOfSuccess, IMissionStep resumeStep)
        {
            ...
            {
                MarkForceLostBehindEnemyLines(context, region, execution.Campaign.Date);
            ...
            {
                DeployForceInTargetRegion(context, region, execution.Campaign.Date);
        ...
        private static void MarkForceLostBehindEnemyLines(
            MissionContext context, Region region, Date currentDate)
        {
            context.ForceLostContact = true;
            MoveForce(context, region, registerAsLanded: false, currentDate);
        }

        private static void DeployForceInTargetRegion(
            MissionContext context, Region region, Date currentDate) =>
            MoveForce(context, region, registerAsLanded: true, currentDate);
