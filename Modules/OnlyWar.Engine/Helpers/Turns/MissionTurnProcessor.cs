...
namespace OnlyWar.Helpers.Turns
{
    ...
    internal sealed class MissionTurnProcessor
    {
        ...
        private readonly CampaignBattleEquipmentSource _equipment;
            ...
            Action<IntelObservation> recordTargetObservation = null)
        {
            ...
            _equipment = new CampaignBattleEquipmentSource(session.Rules, session.Sector.PlayerForce);
            ...
            ICollection<StrategicCombatResult> strategicCombatResults)
        {
            var resolver = new StrategicCombatResolver(
                ...
                strategicInvasionForces: _session.Sector.StrategicInvasionForces);
            ...
            foreach (Order order in combatOrders)
            {
                ...
                foreach (Squad squad in order.AssignedSquads)
                {
                    ...
                    SquadReadinessSnapshot readiness = isPlayerOrder
                        ? SquadReadinessService.Evaluate(squad, program: _session.Sector?.PlayerForce?.RecruitmentProgram, doctrine: doctrine)
                        : null;
                    BattleSquad battleSquad = isPlayerOrder
                        ? BattleSquadFactory.Create(true, squad, doctrine, _session.Sector?.PlayerForce?.RecruitmentProgram, _equipment)
                ...
                involvedBattleSquads.AddRange(order.AssignedCharacters
                    .Select(character => isPlayerOrder
                        ? BattleSquadFactory.CreateAttachedCharacter(
                            ...
                            entityIds.GetNextId(),
                            order.OwnerFaction,
                            doctrine, _session.Sector?.PlayerForce?.RecruitmentProgram, _equipment)
                ...
                if (isPlayerOrder)
                {
                    readinessMessages.AddRange(order.AssignedCharacters
                        .Where(character => !DutyReadinessService.Evaluate(character, doctrine: doctrine, recruitmentProgram: _session.Sector?.PlayerForce?.RecruitmentProgram).IsDutyReady)
                        .Select(character =>
                            $"{character.Name} withheld: {DutyReadinessService.Evaluate(character, doctrine: doctrine, recruitmentProgram: _session.Sector?.PlayerForce?.RecruitmentProgram).Reason ?? "not duty-ready"}."));
                }
                if (involvedBattleSquads.Count == 0)
                {
                    MissionContext unavailable = new(order, [], [], doctrine, _session.Sector?.PlayerForce?.RecruitmentProgram);
                ...
                {
                    MissionContext context = new(order, elementSquads, new List<BattleSquad>(), doctrine, _session.Sector?.PlayerForce?.RecruitmentProgram);
                    ...
                    var execution = new MissionExecutionContext(
                        ...
                        new OnlyWar.Contracts.Operations.MissionCampaignInputs(_session.CurrentDate,
                            _session.Sector.PlayerForce?.Army?.ChapterOperationalDoctrine,
                            _session.Sector.PlayerForce?.RecruitmentProgram,
                            _session.Sector.StrategicInvasionForces, _session.Rules.FactionBehaviorRules, _equipment));
            ...
            ICollection<MissionContext> missionContexts)
        {
            ...
            List<BattleSquad> squads = order.AssignedSquads
                ...
                .Select(squad => BattleSquadFactory.Create(
                    ...
                    squad,
                    _session.Sector?.PlayerForce?.Army?.ChapterOperationalDoctrine,
                    _session.Sector?.PlayerForce?.RecruitmentProgram, _equipment))
