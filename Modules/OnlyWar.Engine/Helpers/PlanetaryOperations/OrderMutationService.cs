...
namespace OnlyWar.Helpers.PlanetaryOperations
{
    ...
    public static class OrderMutationService
    {
        ...
        public static OrderMutationResult Restore(
            Sector sector, OrderRestoreToken token, Date currentDate = null) =>
            token != null && ReferenceEquals(sector, token.Sector)
                ? RestoreParticipants(
                    sector, token.Order, token.Squads, token.Characters, currentDate)
            ...
            Date currentDate = null)
        {
            return CreateOrAdd(sector, target, mission, selectedSquads, [],
                targetFactionId, aggression, currentDate);
        }
            ...
            Date currentDate = null)
        {
            ...
            if (squads.Any(squad => squad.CurrentOrders == null
                && SquadReadinessService.Evaluate(squad, program: CurrentCampaignReadinessContext.ResolveProgram(squad), doctrine: CurrentCampaignReadinessContext.ResolveDoctrine(squad)).PrimaryBlocker
            ...
            Order result = OrderAssignment.AssignParticipantsToMission(
                new OrderCommandContext(sector, currentDate),
            ...
            Date currentDate = null)
        {
            ...
            if (!availability.IsAllowed)
            {
                ...
                OrderAttachment.Attach(soldier, order, doctrine: null, currentDate);
        ...
        public static OrderMutationResult RestoreSquad(
            ...
            Squad squad,
            Date currentDate = null)
            => RestoreParticipants(sector, order, [squad], [], currentDate);
            ...
            Date currentDate = null)
        {
            ...
            foreach (PlayerSoldier character in characters)
            {
                if (character.CurrentOrder != null
                    ...
                    || !DutyReadinessService.Evaluate(character, doctrine, program).IsDutyReady)
                {
                    return Failure("The force changed and a character can no longer join the operation.");
                }
                if (!availability.EvaluateOrderAssignment(character, order, null, staging, doctrine).IsAllowed)
                {
                    if (!OrderAttachment.CanAttach(character, order, staging, null, out string reason, doctrine))
            ...
            foreach (PlayerSoldier character in characters.Where(legacyAttachments.Contains))
                OrderAttachment.Attach(character, order, doctrine, currentDate);
