    ...
    public static class OrderAssignment
    {
            ...
            ChapterOperationalDoctrine doctrine = null)
        {
            ...
            if (squads == null || squads.Count == 0
                || squads.Any(squad => squad?.CanAcceptSquadOrder != true
                    ...
                    || squad.CurrentOrders == null
                        && !SquadReadinessService.CanBeginNewDeployment(squad, program: CurrentCampaignReadinessContext.ResolveProgram(squad), doctrine: CurrentCampaignReadinessContext.ResolveDoctrine(squad, doctrine))))
            {
                return null;
            }
            ...
            if (equivalentOrders.Count > 0)
            {
                ...
                {
                    OrderAttachment.Attach(specialist, existingOrder, doctrine, campaign.PostingDate);
                }
            ...
            foreach (PlayerSoldier specialist in distinctSpecialists)
            {
                OrderAttachment.Attach(specialist, newOrder, doctrine, campaign.PostingDate);
            }
            return newOrder;
        }
