...
namespace OnlyWar.Helpers.Orders
{
    ...
    public static class OrderForceService
    {
            ...
            RecruitmentProgram program = null)
        {
            ...
            DutyReadinessEvaluation duty = DutyReadinessService.Evaluate(character,
                CurrentCampaignReadinessContext.ResolveDoctrine(character.AssignedSquad, doctrine),
                CurrentCampaignReadinessContext.ResolveProgram(character.AssignedSquad, program));
            if (!duty.IsDutyReady)
            {
                return false;
            }
            ...
            RecruitmentProgram program = null)
        {
            ...
            if (squad.CurrentOrders == null
                && !SquadReadinessService.CanBeginNewDeployment(squad,
                    CurrentCampaignReadinessContext.ResolveProgram(squad, program),
                    CurrentCampaignReadinessContext.ResolveDoctrine(squad, doctrine)))
            {
                return false;
            }
