...
namespace OnlyWar.Helpers.Battles
{
    public class BattleSquad : ICloneable
    {
            ...
            IEnumerable<ISoldier> engagementParticipants = null, IBattleEquipmentSource equipment = null)
        {
            ...
            Soldiers = squad.Members.Where(member => member is not PlayerSoldier player || player.IndividualPosting == null)
        ...
        public BattleSquad(BattleElementSpec spec, IBattleEquipmentSource equipment = null)
        {
        ...
        /// </remarks>
        public void ReallocateEquipment()
        {
        ...
        private void AllocateEquipment()
        {
            ...
            List<WeaponSet> wsList = (_equipment?.GetSquadLoadout(Squad) ?? Squad.Loadout ?? [])
        ...
        private void MarkEquipmentValueSource(BattleSoldier soldier)
        {
            ...
            soldier.UsesItemizedEquipment = _equipment?.UsesItemizedAttribution == true;
        }
        ...
        private WeaponSet GetCharacterWeapons(ISoldier soldier)
        {
            if (_equipment != null) return _equipment.GetCharacterWeapons(soldier);
            var element = soldier?.AssignedSquad?.SquadTemplate?.Elements.FirstOrDefault(e => e.SoldierTemplate == soldier.Template);
            return element?.TryGetQuota("Command Weapon", out _) == true
                ? element.DefaultWeapons ?? soldier.AssignedSquad.SquadTemplate.DefaultWeapons : null;
        }
        private WeaponSet ResolveElementDefaultWeapons(ISoldier soldier)
        {
            if (Squad == null)
            {
                return GetCharacterWeapons(soldier)
        ...
        private void AllocateCharacterEquipment(List<BattleSoldier> tempSquad)
        {
            for (int i = tempSquad.Count - 1; i >= 0; i--)
            {
                ...
                WeaponSet weaponSet = GetCharacterWeapons(battleSoldier.Soldier);
                if (weaponSet == null)
                {
                    continue;
                }
            ...
            out Armor armor)
        {
            ...
            ResolvedEquipmentLoadout resolved = _equipment?.Resolve(battleSoldier.Soldier, Squad, battleSoldier.FunctioningHands);
