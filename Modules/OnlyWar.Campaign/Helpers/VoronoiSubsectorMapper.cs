...
namespace OnlyWar.Campaign
{
    ...
    public static class VoronoiSubsectorMapper
    {
        ...
        {
            public Dictionary<ushort, List<PlanePoint[]>> Loops { get; } = [];
            ...
            int maxDiameter)
        {
            ...
            foreach (var kvp in regionAdjacency)
            {
                ...
                List<PlanePoint[]> loops = ChainLoops(kvp.Value, triangles);
                if (loops.Count > 0)
                {
        ...
        private static List<PlanePoint[]> ChainLoops(Dictionary<int, List<int>> adjacency, List<Triangle> triangles)
        {
            List<PlanePoint[]> loops = [];
        ...
        private static PlanePoint[] BuildSmoothedLoop(List<int> loopNodes, List<Triangle> triangles)
        {
            PlanePoint[] rawPoints = new PlanePoint[loopNodes.Count];
            for (int i = 0; i < loopNodes.Count; i++)
            {
                Triangle tri = triangles[loopNodes[i]];
                rawPoints[i] = new PlanePoint((float)tri.CircumX, (float)tri.CircumY);
            }
            return ChaikinClosed(rawPoints, ChaikinPasses);
        }

        private static PlanePoint[] ChaikinClosed(PlanePoint[] points, int passes)
        {
            PlanePoint[] current = points;
            for (int pass = 0; pass < passes && current.Length >= 3; pass++)
            {
                PlanePoint[] next = new PlanePoint[current.Length * 2];
                for (int i = 0; i < current.Length; i++)
                {
                    PlanePoint a = current[i];
                    PlanePoint b = current[(i + 1) % current.Length];
