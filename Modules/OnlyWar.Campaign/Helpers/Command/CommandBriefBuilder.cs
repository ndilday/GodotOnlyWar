...
namespace OnlyWar.Helpers.Command
{
    ...
    /// </summary>
    public sealed class CommandBriefBuilder
    {
        public CommandBriefModel Build(
            Date currentDate,
            Sector sector,
            GameRulesData rules,
            ...
            Sector sector)
        {
            ...
            List<PlayerSoldier> wounded = force?.Army?.OrderOfBattle?.GetAllMembers()
                ...
                .Where(soldier => !DutyReadinessService.Evaluate(soldier, doctrine: force?.Army?.ChapterOperationalDoctrine, recruitmentProgram: force?.RecruitmentProgram).IsDutyReady)
