...
**Pursuit fire-window policy.** The Xibarrus pursuit review established three linked rules. First, ranged pursuit's Run term is continuous across the preferred band, so there is a reason to run outside the band and no one-yard score cliff at its edge. **The form of that term was replaced on 2026-09-20 and the original rule is superseded.** It used to be zero at `PreferredBandLower`, full at `PreferredBandUpper`, interpolated between them, then multiplied by the fraction of net closing speed the candidate used — and that last multiplier made a state potential depend on the speed that reached the state. The root state is always evaluated at zero speed, so the root scored zero, every moving candidate scored the squad's whole attainable Battle Value, and `gamma*Phi(s') - Phi(s)` never telescoped: it became a per-turn bounty for moving that no amount of closing reduced. Measured at exactly 141.150 for a running squad in two windows seventy turns apart while the separation closed, against a standing shot worth 7.4. Contact progress is now a saturating position value, `attainable * span / (span + excess beyond the desired range)`, worth `attainable` across the whole approach rather than every turn, with the quarry's speed moved out to the terms that price time. Continuity and the absence of a cliff are preserved; the taper across the band is not, and the marginal value of a yard now rises as a squad closes. See `Design/Reference/BattleLogic.md` §5.2. Second, a Pursuit `Hold` candidate receives a separate `FireWindowValue`. It projects the quarry opening at its declared run speed for `PursuitFireWindowTurns = FullAimBonusTurns + 2` (five turns: four fresh `Aim` actions followed by the full-aim shot), evaluates each shooter's actual weapon/target choice at that projected range with `Accuracy + FullAimBonusTurns + 1`, and discounts the resulting expected removal with the normal continuation discount. The value is zero when no positive, worthwhile full-aim shot survives that projection, so a squad must run until the target will still be a viable shot at the end of the cycle. `ENGAGE_EVAL` exposes this term as `fire_window`.
...
meaningful positive closing speed, a worthwhile attack executable now, or a committed fire cycle
making observable progress toward a worthwhile attack. A theoretical shot bridges non-attacking
turns only while the squad holds, retains a viable target, and accumulates readiness; it cannot keep
contact alive while the chosen policy repeatedly declines that fire plan. `Standoff` satisfies this
through its hold-and-fire constraint, while `Follow` may instead satisfy it by genuinely closing.
Force-level capability does not replace pair-local progress.
