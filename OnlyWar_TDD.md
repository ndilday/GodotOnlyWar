...
Order                (Id, MissionId, Disposition, IsPlayerControlled,
                      IsForceAdvance, LevelOfAggression)
...
SoldierEvaluation    (SoldierId, EvaluatingSoldierId, Millenium, Year, Week,
...
### 8.1 Duplicate Mission Save — Bug (High)

**Location:** `PlanetDataAccess.SavePlanetRegions` and `PlanetDataAccess.SaveMissions`

`SavePlanetRegions` contains an inline loop that inserts rows into the `Mission` table. `SaveMissions` is then called immediately after from `SavePlanet` and inserts the same rows again. The current schema has no UNIQUE constraint on `Mission.Id`, so both inserts succeed, leaving duplicate rows. Loading will produce duplicate special missions on those regions. This will become a hard failure the moment a UNIQUE constraint or foreign-key reference to `Mission.Id` is added.

**Fix:** Remove the mission insert loop from `SavePlanetRegions`. Add a UNIQUE constraint on `Mission.Id` to the schema.
...
### 8.5 String-Interpolated SQL — Medium

**Location:** All `DataAccess` classes

All SQL is built via string interpolation. Any string value containing a single quote will break the insert (partially addressed in `SoldierDataAccess.SaveSoldier` with a manual `Replace("'", "''")` but not consistently applied). Float locale issues are also a latent risk on non-English systems.

**Fix:** Migrate to parameterized queries using `SqliteCommand.Parameters`. This also removes the SQL injection surface area (low concern in single-player, but good hygiene).
