...
public class MoveActionTests
{
    ...
    public void Execute_SubsequentMoveCanSpendAccumulatedFractionalBudget()
    {
        ...
        ExecuteMove(soldier, grid, new ValueTuple<int, int>(1, 0), movementBudget: 1.2f);
        Assert.Equal(0.2f, soldier.LeftoverMovement, precision: 4);

        ExecuteMove(soldier, grid, new ValueTuple<int, int>(2, 0), movementBudget: 1.2f + soldier.LeftoverMovement);
        Assert.Equal(0.4f, soldier.LeftoverMovement, precision: 4);

        ExecuteMove(soldier, grid, new ValueTuple<int, int>(3, 1), movementBudget: 1.2f + soldier.LeftoverMovement);

        Assert.Equal(new ValueTuple<int, int>(3, 1), soldier.TopLeft);
