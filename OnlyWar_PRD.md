...
- Equipment capacity is data-driven in points from species, role, armor, and gear. Every carried item/quantity has a cost; the UI and save model must not hard-code an entry count or allow an unbounded list.
- Items carry eligibility and interaction metadata: species/faction restrictions, handedness, armor compatibility, role requirements, duplicate limits, and any capacity or bulk cost. Invalid combinations are rejected before battle with an explicit reason.
- Loadouts persist per character through squad transfers and save/load. Personal overrides are complete loadouts and inherit only when absent; an invalid active override blocks deployment and offers an explicit reset rather than being silently overwritten. Mission equipment is instantiated once, survives repeated battles with its ammunition state, and casualty replacement moves the same recoverable item to the best valid surviving carrier while reporting equipment that cannot be carried.
- The Chapter Screen and Soldier Screen expose the same loadout state. The player can see occupied hand groups, armor, equipment capacity/contents, restrictions, and the resulting ranged/melee capability before committing the squad to a battle.
- The existing pooled squad interaction remains standard issue plus specialist-kit counts. Itemized editing is initially exposed for authored command/specialist roles, so this feature does not require the player to equip ordinary line Marines individually.
...
- Wounds heal a week at a time during turn processing, for every wounded marine regardless of deployment — **except** severed non-vital locations that require a replacement procedure, which do not heal on their own and stay out-of-action until treated. Crippled locations do not require replacement for now.
...
*Built in 0.7.3; the aggregate and weekly processing model are in TDD §5.9. One gap: only the Chapter Home World is a recruiting source, so the "per-world rights granted by a governor relationship or manpower pledge" bullet below is specified but not yet built.*
...
- **Wounds are discrete injuries, not a single severity counter.** Every active severity band a location is carrying convalesces *concurrently* on its own clock and heals one band at a time — a Major wound becomes Moderate, then Minor, then nothing. A location's lesser wounds never wait on its worst one, and empty bands do not accumulate healing progress. The implementation invariant and 0.7.3 cadence correction are recorded in TDD §5.3.
...
**Battle Loadouts**
- Equipment is allocated from the current doctrine and character kits at the start of each battle. Under the 0.8 bespoke-loadout model (§4.5), valid per-character assignments take priority, then explicit squad doctrine, then the template default. If a casualty leaves a weapon or equipment item uncarried, the next battle allocates it to the best eligible surviving carrier and reports anything that cannot be carried; there is no separate post-battle mutation pass.
...
| In Melee | Per soldier | No squad-wide aim behavior | Per the soldier's effective movement and engagement | Yes | Adjacent soldiers hold and fight; separated soldiers close, striking next turn |
...
- **Turn order.** Attacks resolve before movement, and both sides declare from the same turn-start positions. Ranged fire resolves first, then melee, then every squad moves. So a soldier attacks from where he stood when the turn opened, never from where he ends up, and melee contact is a fact established by the *previous* turn's movement rather than something this turn's movement can create. This is what keeps ranged geometry and melee adjacency the same kind of fact; without it melee alone needs re-planning mid-turn.
- **Charge.** A charge is one act split across the turn boundary, because closing and striking cannot share a turn. A soldier who reaches contact during a turn's movement delivers his charging blow at the top of the next turn — nothing happens in between, since movement ends the turn and attacks open the next. That blow uses the existing −2 moved-attack accuracy penalty, and the charger forfeits all weapon `ParryModifier` benefit for that turn, but receives no additional defensive penalty because their movement speed already makes them harder to hit at range. A soldier who has held contact since before the previous turn is fighting set, not charging, and pays neither.
- **In Melee.** This tier is a squad tactical mode rather than a uniform movement rate. A soldier already adjacent to an enemy stays in place and chooses a legal melee or point-blank action. A separated soldier moves toward an available engagement position, using up to Run allowance; reaching contact commits him to a charging blow next turn, and if he cannot reach contact he continues closing without ranged fire. The selected squad tier is stored in battle state, while `CurrentSpeed` and `LeftoverMovement` remain per-soldier state.
...
- Player-soldier incapacitation and battlefield-control recovery follow the three-state rules in §4.12; battle results record incapacitated brothers separately from killed brothers.
...
- **Rout:** rout is an involuntary morale state, not a selected withdrawal plan. A routing squad Runs without covering fire or orderly role rotation and remains vulnerable to interception. Rout reuses the withdrawal/pursuit state and aftermath machinery. Triggers are decided (§6.2, resolved 2026-07-16, revised 2026-09-24): an event (casualties, leader or command loss, synapse loss, a friendly rout) triggers a check; the check rolls a stress score per soldier against a resolve curve and aggregates to a squad-level Steady/Shaken/Routing outcome, with Synapse-covered squads skipping the check entirely and Shaken squads rolling to rally on quiet turns.
...
- The fleet transfer tree supports drag-and-drop of squads and whole units between ships; legality is checked during the drag and an invalid drop leaves the assignment unchanged.
...
- **Soldier History Log (4.7):** The event vocabulary is enriched beyond recruitment/promotion/wound to include first blood, survival against odds, instructor/mentor relationships, kill milestones, near-death recoveries, and significant body-part replacements (cybernetic or vat-grown).
...
**Committed Feature — Command Brief & Chapter Chronicle.** This is one persistent command surface with two complementary lenses over the same typed event/state substrate. The bottom-menu Archive destination becomes **Command**, opening the Brief by default; the saved Last Turn Report remains available from the workspace header:
...
- **Chapter Chronicle (past):** a persistent sector-level narrative log that records notable events from the classifier into one readable saga — founding myth, defining battles, named heroes and their deaths, worlds won and lost, factions revealed, and mandates completed or failed. Routine battles remain in the Turn Report; correlated notable facts compose into one battle account by default, while genuinely defining personal events may stand alone.
- The first turn opens the Command Brief as lightweight onboarding instead of presenting a separate one-shot briefing modal. A revisitable Founding Directive and state-derived checklist point the player toward the Promised World, landing forces, issuing orders, and understanding End Turn without creating a separate tutorial mode or persisted screen-visit flags.
...
**Insurrectionist Force Identity**
- Insurrectionists use their own species, soldier templates, weapon sets, and formations rather than cloning the PDF. Their wider attribute variance and lower Generic Ranged training make the average insurgent no cheaper than a PDF trooper while producing a less uniform rabble.
- The faction fields three formations: a Scout-flagged Mob with a Ringleader and 4–29 rolled insurgents, a two-man Weapon Team sharing a heavy stubber, and a single Firebrand HQ with a Mob bodyguard. The faction uses autoguns and no grenades; the Firebrand supplies its command presence.
...
**Description.** The former simulation determined hostility with a single binary test: a faction was either Imperial-aligned (player or default) or not, and the two sides were enemies. That was sufficient while every non-Imperial faction was a monolithic "them," but it could not express a sector with **multiple mutually hostile non-Imperial factions** — Orks that fight everyone including other xenos, a Chaos world that also carries Ork spores, a renegade revolt that is no friend of the Tyranids. The implemented relationship and intelligence substrates below now support those cases and are shared by Orks, Revolt, and future Chaos:
...
- `FactionRelationshipService` resolves this store (Hostile ⇒ targetable/contested; Neutral ⇒ non-targetable but non-sharing; Allied ⇒ non-targetable and sharing), and offensive selection, required-garrison threat assessment, patrol targeting, fortifications, and intelligence/visibility route through the appropriate effective stance query.
...
- Intelligence is modeled as **belief held by an observer**, not as a direct property of the target. The Alpha 0.8 implementation uses the full form keyed `(observer Faction, target Faction, Region)` and carries a **believed presence/strength** plus an **`IntelLevel`**: `None | Rumor | Suspected | Confirmed | Located`. The existing sparse numeric `PlanetFaction.RegionAwareness` value is renamed **regional awareness** and remains a separate target-agnostic value describing how well that faction understands and watches the region; target-specific beliefs use it as an observation input but do not derive truth from it. Consumers read stored belief, not ground truth.
...
- ✅ **Insurrectionist irregular squad types** — faction-owned species, formations, weapons, and opt-in rolled mob strength replaced the PDF clone. §4.20; TDD §§5.4, 6.7. Godot verification of encounter presentation remains pending.
...
standard-issue/specialist-count editor still reads the established `WeaponSet` surface so existing
formation-quota interactions remain stable; migrate that remaining pooled menu and its legacy doctrine
tables to kit ids in a later cleanup, then remove the compatibility type. This residual is intentionally
tracked here rather than kept as an active design document.

**Melee closing tax recalibration.** `BattleValueCalculator.MeleeClosingFactor` is 0.75 and is now
optimistic. Reaching contact no longer buys a blow in the same turn (§4.14), so every closer waits one
more turn to land anything, per engagement rather than per battle. The factor is a flat multiplier
standing in for the fraction of a soldier's turns spent swinging, so nothing breaks and no corrected
value follows from the rules change — it abstracts away the very turn count that moved. Two properties
make this a balance pass rather than a dial: the extra turn is a fixed +1 regardless of speed, so it is
...
- **Trigger — events decide *when* a squad checks; the stress score decides *how hard* the check is** (revised 2026-09-24). A squad checks at the end of a turn only when one of these happened: it took casualties; its squad leader was lost; its side's last command squad was destroyed; its synapse coverage was lost; or a friendly squad, at any range, routed on the previous turn (squads share a vox net). On a quiet turn it rolls nothing. The stress score is unchanged and still recomputed from current state, so cumulative casualties and command loss still make every triggered check harder. The original design checked every turn; that let permanent terms (cumulative casualties, a dead Warboss) make a battered squad fail on turns when nothing happened to it, over and over. The old objection to event triggers — a squad losing 10% a turn for six turns never checks — does not apply, because every casualty triggers a check (`MoraleConstants.CasualtyCheckStrengthFraction` = 1.0; lower it to make casualties trigger only below a fraction of starting strength). Global context **multiplies** local shock rather than adding to it, so the first casualty when the line is collapsing outweighs the third when the force is winning.
...
- **Outcomes — Steady / Shaken / Routing.** Shaken (will not advance, degraded fire) keeps most checks from being binary catastrophe. Routing reuses the behavior already specified in Battle Continuation (§4.14). **Rally** (2026-09-24): a Shaken squad with no trigger rolls against its current stress each turn; a pass makes it Steady and a fail leaves it Shaken — a rally roll never makes a squad worse, so it never routs one and never calls on Ork leader coercion. **Broken** (post-battle unavailability) remains deferred; it needs strategic-layer UI.
- **Propagation — a trigger plus a stress input, not a contagion roll.** A friendly squad routing anywhere on the field triggers a check for the rest of its side on the next turn. A friendly squad routing within visual range also raises stress, read from the turn-start snapshot. Both read the previous turn, so iteration order is irrelevant and same-turn avalanches cannot occur. Cascades emerge without a second subsystem.
...
| Regional Awareness | A sparse, target-agnostic per-observer/per-region value describing how well a faction understands and watches that region. It improves observation quality and defensive surveillance, decays over time, and does not itself reveal a target (§4.21). |
| Listening Post | A buildable/sabotageable regional sensor infrastructure value on `RegionFaction`, replacing the old `Detection` defense. It feeds the owning faction's regional awareness each turn rather than being target intelligence itself. |
...
| In Melee | A squad-level movement tier for maintaining and completing an engagement. Soldiers already adjacent to an enemy hold and fight; separated squad members close toward an engagement position, and a soldier who reaches contact delivers his charging blow at the top of the next turn. |
