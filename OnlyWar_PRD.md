...
- Replace the current squad-template-only loadout assumption with a character-level equipment model. Squad templates continue to provide role defaults, eligibility, and formation constraints, but each character has an explicit loadout that can be reviewed and edited before deployment.
- Each character loadout has four parts: one **primary hand-group entry** that accepts either a one-handed or two-handed item; one optional **single-hand entry**; one **armor entry**; and a collection of **equipment entries** for items such as grenades, tools, relics, ammunition carriers, or faction-specific gear.
- The primary hand-group entry may hold a one-handed weapon or a two-handed weapon. A two-handed item occupies both hand groups and makes the single-hand entry unavailable; two compatible one-handed items may be carried together when the character, species, and rules data permit it. The model must use the existing hand-function and disabled-limb rules in battle rather than treating loadout slots as cosmetic labels.
- Equipment capacity is data-driven per character, species, armor, or kit. The initial 0.8 implementation should use a finite default capacity (the exact default, such as three entries, remains a balance decision); it must not hard-code either an arbitrary global maximum or an infinite unbounded list into the UI or save model.
...
- Loadouts persist per character through squad transfers and save/load. Battle-start allocation may fill an unconfigured character from squad doctrine or a template default, but it must not silently overwrite a valid bespoke loadout. Casualty replacement chooses the best valid surviving carrier and reports any equipment that cannot be carried.
...
- **(E) Equipment-aware tactical Battle Value** — separate a soldier's stable, intrinsic/template Battle Value from the tactical value of the equipment actually assigned to that soldier. Recalculate `SoldierTemplate.BattleValue` from values intrinsic to the template (species attributes, training profile, and other template-owned capabilities), and introduce a runtime **effective Battle Value** that adds the contribution of the equipped armor, weapons, and relevant gear after loadout resolution. A Brood Brother carrying the autocannon in a Brood Brother Weapon Team should therefore have a higher effective tactical value than an otherwise equivalent Brood Brother in a regular squad. Compute and cache effective values once per equipped soldier or loadout signature; refresh them when battle-start allocation, casualty replacement, or a bespoke loadout changes the carrier. Tactical target selection, expected friendly-value loss, and remaining-force/threat calculations use effective Battle Value, while strategic force generation, mission sizing, and persistent casualty accounting continue to use the intrinsic/template value. The effective value is derived runtime state rather than a second authoritative persisted point value, and pooled weapons are attributed to their current carrier. Update generated data and balance baselines as part of the migration, and cover both the Brood Brother comparison and carrier reassignment in deterministic tests.
...
| Character Loadout | A persistent per-character equipment assignment consisting of a primary one-or-two-hand entry, an optional one-hand entry, armor, and data-driven equipment entries (§4.5). |
