...
- **Gated follow-ons (Battle Logic Phase 4, §5.4).** An "on fire" damage-over-time and panic condition remains deferred now that the shared morale system is implemented (`OnlyWar_TDD.md §6.6`). Fire panic should feed that morale stress function as another shock term rather than becoming a separate escape system. Cover/terrain interaction with templates is gated on Battle Visuals Phase 3 line of sight.
...
- **Battle Logic Phase 4B:** Morale, on-fire damage-over-time/panic (§4.14 gated follow-on), Rout behavior, and post-battle loadout recalculation. *(The organized covered-withdrawal/pursuit/rear-guard system and immediate Burrow disengagement are implemented. Flamers/cone templates were pulled forward to §5.2; grenades and Phase 4A movement tiers shipped in §5.3.)* Sub-item status:
  ...
  - ⬜ **On-fire damage-over-time / panic.** Not started; no burning condition exists in the codebase. Per §4.14, fire panic should feed the existing morale stress function as another shock term rather than become a separate escape system.
...
- **Insurrectionist irregular squad types:** The Insurrectionists ship with no squad templates of their own and currently borrow a clone of the PDF Infantry Squad, so a revolt fields forces indistinguishable from the PDF it rose against. Give them at least one or two squad types of their own that read as an irregular, untrained rabble — poorer equipment, looser organization, and larger/less consistent squad sizes than a drilled PDF formation — rather than reusing the state's own line infantry. *(Not started — confirmed in the rules DB: faction 5 still has exactly one `SquadTemplate`, `Insurrectionist Infantry Squad`, alongside the Imperial faction's single `PDF Infantry Squad`.)* *(Interim: a faction with zero squad templates got a **null** `SquadTemplates` map from `GameRulesDataAccess`, which every `ForceGenerator` profile then dereferenced; the generator now normalizes null → empty, so a template-less faction degrades to "fields no force" instead of throwing.)*
