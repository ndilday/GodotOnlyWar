...
- A new wound to a location resets that location's healing clock across all severity bands, because healing progress is tracked per location rather than per wound — see §6.14 for why that is the coherent rule and what a graduated alternative would look like.
...
- **Leg Wound & Prone-Combat Realism (maybe):** Rework the leg-hit outcome so a single solid hit staggers rather than reliably felling. Motivated by combat GSW data: even for 40k-tier energies (nothing softer than .45/7.62), only ~45–55% of solid leg hits fracture bone/joint or major vessel and actually prevent movement; the rest slow but don't stop. Scope:
  - **Raise the "can no longer walk" bar one level to Massive.** *(Not started — verified in the rules DB: both `Left Leg`/`Right Leg` rows still carry `CrippleWoundLevel = 65536` (`WoundLevel.Critical`), one band below `Body`'s 1048576. A single DB update is the whole mechanical change; the sub-items below are the real work.)* Motive (leg/foot) locations currently drop a soldier at their *cripple* threshold (Critical, i.e. damage ≥ Constitution); require the higher *Massive* band instead so a Critical leg wound impairs but does not fell. Torso/vital lethality is unchanged; this narrows the current asymmetry where a leg is fight-ending at half the damage the torso needs to be decisive.
...
- **Player-Soldier Incapacitation (maybe):** *(Not started — "incapacitated" exists only as a transient in-battle predicate derived from `CanFight`, with no persistent casualty state distinct from killed.)* Add *incapacitated* as a persistent casualty state separate from both immobility and death, reflecting an Astartes' superhuman ability to survive wounds that end his participation in a battle. This creates three distinct outcomes for player soldiers:
  - **Unable to walk:** the soldier cannot move normally, but may remain conscious and combat-capable (including firing prone under the Leg Wound rules above).
  - **Incapacitated:** the soldier is alive but unable to continue fighting and is removed from combat. His wounds and incapacitated state persist into the battle result rather than being reported as a kill.
  - **Killed:** the soldier is dead and follows the existing fallen-brother, gene-seed, and casualty-record rules.
  - **Recovery requires control of the battlefield.** If the player wins, incapacitated soldiers are recovered for treatment and enter the existing wound-recovery pipeline; recovery time and any lasting injury follow their actual wounds. If the player does not win, they are not automatically recovered—their final disposition must be resolved by the withdrawal/battle-outcome rules rather than silently returning them to the roster.
