...
6. [Open Design Questions](#6-open-design-questions)
   ...
   - 6.3 [New Recruit Intake — V1 IMPLEMENTED IN 0.7.3](#63-new-recruit-intake--v1-implemented-in-073)
...
**Screen structure (Implemented).** The visual direction is the V2 flow in `Design/VisualBaselines/Apothecarium/`. A persistent left panel holds a default-selected **Gene Seed Vault** button above a wounded-filtered chapter/unit tree; a stateful detail panel to its right shows one of three views:
- **Vault** — mature gene-seed stockpile, mature and immature implanted progenoids, progenoids maturing within one year, at-risk implanted progenoids, aggregate gene-seed purity with a derived quality label (Pristine/Stable/Degraded/Corrupt), and the current Requisition balance.
- **Unit/squad rollup** — medical readiness before drilling into individuals: healthy, wounded, out-of-action, ready-next, maximum recovery time, and serious-wound rows.
- **Soldier medical** — wound locations and severity, expected recovery time, whether gene-seed-bearing locations are safe/damaged/lost, replacement eligibility, and treatment options. Includes a **Vitruvian wound diagram** mapping hit-location names to stable body coordinates, coloured by wound severity, severed state, crippled state, and cybernetic replacement.
...
- Wounds heal a week at a time during turn processing, for every wounded marine regardless of deployment — **except** locations that require a replacement procedure (severed, or a crippled functional/vital location), which do not heal on their own and stay out-of-action until treated.
- Displays the current count of mature geneseed stored in the Reclusiam.
- **Gene-seed purity is tracked as real aggregate data**, not a presentation label: the chapter carries a count-weighted purity that each recovered gland folds into, rolled around a baseline with a small downward drift. Purity is tracked and displayed here; it is *consumed* by the recruitment pipeline (§4.9).
...
**Apothecary field care (Implemented).** Weekly natural healing above is unconditional and applies to everyone; an Apothecary's presence *accelerates* recovery and is never a prerequisite for it. Attaching an Apothecary to an operation (§4.13) — or leaving him at home — is a real allocation decision with both effects visible on this screen.

- **Treatment is immediate, not banked.** An Apothecary spends a daily capacity, derived from his **Medical** rating, on reducing his patients' wound severity, and the reduction applies the day it happens. A brother wounded on day 2 of a week-long assault and treated that evening goes into the day-3 fight in better shape. This is what makes a field Apothecary feel different from a week of bed rest.
- **Reach is the operation.** He treats the wounded of every squad under the same order, plus anyone else attached to it. An Apothecary who stays behind instead treats co-located brothers who are not deployed — the Apothecarium at rest, where most convalescence actually happens.
- **The tension is legible.** An Apothecary sent forward with an assault is an Apothecary not clearing the backlog at home, and the screen says so: brothers he no longer covers read as having no Apothecary on hand.
- **Worst first, deliberately.** Capacity concentrates on the men who would otherwise be out for months rather than being spread thinly across light wounds — Astartes physiology handles those without help. Priority is severity first, then seniority. The severity used is the same expected-recovery-time figure shown to the player, so the order displayed is the order the game runs, and it re-evaluates continuously as men improve.
- **He cannot substitute for surgery.** A wound severe enough to require a replacement procedure is beyond field care; it still needs an Apothecary, a Techmarine, a surgery site, and Requisition. In practice this means field care serves the walking wounded, and a brother who actually went down is a surgical case.
- **Treating is practice.** An Apothecary develops his medical skills by working, in proportion to the care he actually delivers — a busy week advances him comparably to a week of training. An idle one learns nothing.
...
*Built in 0.7.3; see §5.4 for the implementation note. One gap: only the Chapter Home World is a recruiting source, so the "per-world rights granted by a governor relationship or manpower pledge" bullet below is specified but not yet built.*
...
- Three staff roles run the program, each owning a distinct screen and a distinct capacity:
  - **Scout Sergeants** conduct the non-genetic testing — physical and martial aptitude — and supply aspirant training capacity. Their **Leadership** rating drives both.
  - **Apothecaries** conduct genetic screening and perform the implantation procedures. Their **Medical** rating drives both.
  - **Chaplains** conduct the spiritual screening and supply their own share of aspirant training capacity. A Chapter does not admit a boy on physique and gene-compatibility alone; someone must judge whether he can bear what is about to be done to him and remain the Emperor's. Chaplain effectiveness is rated on **Piety** weighted with **Leadership**, since the role is equal parts judgement of character and instruction. **Judiciars** are eligible for this role on the same terms.
- The relevant skills of assigned personnel affect coverage and capacity, creating a choice between committing the Chapter's best personnel to recruitment or deploying them to battle.
- **Each role gates the program independently — capacities do not substitute for one another.** Weekly screening throughput is the *minimum* of non-genetic, genetic, and spiritual screening capacity, and aspirant training capacity is the *minimum* of Sergeant and Chaplain capacity. A candidate who has not been screened on all three axes is not screened, and an aspirant no one is available to instruct is not trained. Consequently the program cannot operate at all without at least one Scout Sergeant, one Apothecary, and one Chaplain assigned, and the founding setup requires all three before it can be confirmed. This makes staffing a genuine allocation problem rather than a single dial: over-assigning one role buys nothing until the scarcest role is also reinforced, and the Recruiter must show which role is currently the binding constraint.
...
- The program has a modest operating cost. The initial v1 tuning target is **1 Requisition per assigned staff member per week**, charged the same for all three roles; the Recruiter displays the current and projected weekly cost. If the Chapter cannot pay the week's cost, the program pauses rather than running unfunded.
- Candidate attribute generation is source-world-aware. Children from **Feral** and **Death** worlds have higher baseline attributes than children from other world types, reflecting the hardier stock of families able to survive and raise children in those environments. The world-type modifier applies both when individual candidates are generated and when the Recruiter forecasts attribute-screening pass rates; its exact magnitude is a centralized tuning value.
...
- Claiming the Promised World launches a dialog chain that establishes the initial recruitment policies and assigns the initial Sergeants, Apothecaries, and Chaplains. Because all three roles gate the program, the setup cannot be confirmed until at least one of each is assigned. It previews both the finite number of existing survivor children expected to qualify and the sustainable rate of newly eligible children.
...
**Description.** A post-battle replay/report screen for understanding what happened in a completed engagement. It is explicitly **not** a tactical command surface — battles have already resolved — so it avoids order buttons, command rails, movement previews, and territory-ownership washes. The visual direction is the V3 Chronicle Formation Hybrid baseline in `Design/VisualBaselines/BattleReview/`.

**Acceptance Criteria (Implemented — 0.7):**
- The screen uses a **four-region layout**:
  - **Left** — a collapsible force hierarchy tree: opposing forces, nested companies/squads/vehicles, current and starting strength, losses, selection highlighting, and faction/type icons. Selecting a squad row selects that formation.
  - **Center** — a replay viewport drawing the battlefield grid, per-soldier formation markers coloured by affiliation, centroid formation labels with live strength, formation banners, casualty and rout markers, and projectile/charge/melee callouts. Markers are clickable to select a formation. Compact playback controls sit top-center.
  - **Right** — the selected formation's summary (name, type, force, commander, starting/current strength, losses with percentage, and derived fatigue/morale/ammunition labels plus notable effects), above an event chronicle of timestamped actions, morale checks, routs, volleys, casualties, and phase summaries.
  - **Bottom** — an informational battle timeline (each round seekable, severity-tinted) and a casualties-by-round table showing per-round and cumulative losses per side. Not a command menu.
- **Playback**, not just stepping: previous round, step back, play/pause, step forward, next round, and a speed selector (0.5×/1×/1.5×/2×). Play advances rounds automatically on a speed-scaled time budget. Step controls are disabled at the ends of the battle.
- The screen preserves the visual language of the Sector Map and Chapter Screen — dark panels, antique-gold borders, parchment text, smoky glass, muted cyan/crimson reserved for unit affiliation, amber for warnings and notable events — drawn from the shared theme helpers rather than per-screen literals.
...
- **Wounds are discrete injuries, not a single severity counter.** Every severity band a location is carrying convalesces *concurrently* on its own clock, and heals one band at a time — a Major wound becomes Moderate, then Minor, then nothing. A location's lesser wounds never wait on its worst one, in the same way a split lip does not wait on a broken nose. *(Corrected in 0.7.3: the clock advanced for unoccupied bands as well, so a wound stepping down found the band below had already served its dwell time and fell through to Minor in a single pass. Every wound above Moderate was healing in roughly a quarter of its advertised time. Fixed in `Wounds.AdvanceOccupiedBandClocks`, with the cadence pinned by `WoundHealingCadenceTests`.)*
- A new wound to a location resets that location's healing clock across all severity bands, because healing progress is tracked per location rather than per wound — see §6.14 for why that is the coherent rule and what a graduated alternative would look like. Note the interaction with sustained fighting: a brother scratched repeatedly across a week-long assault may accumulate little natural healing, which is precisely the condition that makes an attached Apothecary (§4.8) matter most.
- **Astartes close negligible wounds overnight.** At the end of each campaign day, the lightest wounds clear on their own for Astartes, reflecting their accelerated healing. The boundary is meaningful rather than cosmetic: a *day's* worth of glancing hits no longer compounds into a real wound, while a single battle's worth still can, since a battle resolves inside one day. Minor and above stay on the weekly cascade.
...
- After the mission objective is resolved, the squad exfiltrates (if in an enemy region). Exfiltration uses the same region-wide search-effort model as infiltration.
...
- **Battle Value.** Template weapons are valued by expected victims per burst against the reference threat panel — density-scaled (multiple chaff caught per burst, one monster) — with the ammo/reload duty cycle feeding the standard BV = k·√(offense × durability) pipeline.
- **No burning condition — rejected, not deferred (2026-08-08).** Flamers inflict their damage at the moment of the burst and nothing afterward. There is no "on fire" state, no damage-over-time, and no fire-specific panic. The GURPS-style rule this was modeled on (3–9 points through armor costs the victim their next turn beating out the flames; 10+ means three seconds prone) is load-bearing on assumptions this setting does not share, and rejecting it is a design decision rather than a scheduling one:
  - **Its real payoff is action-economy denial, and that is expensive state for a small effect.** A persistent condition would have to be tracked across turns, serialized and save-migrated, replayed in the battle viewer, and priced by `BattleSquadPlanner` on both sides (is setting a target alight worth it; is extinguishing worth a turn). The flamer's distinctiveness is already delivered up front by the indiscriminate auto-hit cone above; it does not need a second mechanic to be worth carrying.
...
Blast templates extend the cone-template machinery with a second delivery mode: a circular blast of `AreaRadius` centered on an **impact point**, delivered either **thrown** (frag grenades) or **launched** (the Grenade Launcher, currently a Genestealer Cult weapon). The implementation architecture is documented in TDD §6.6; remaining follow-ups are tracked in `Design/Active/RangedCombatFollowUps.md`.
...
**Implementation status (post-0.7).** Organized withdrawal, pursuit, rear guards, Burrow disengagement, and involuntary morale routing are implemented end-to-end, including typed outcomes/events, decision logging, replay, mission aftermath, Synapse, command auras, and forecast-aware rear-guard selection. Flight will use the same capability seam once flying units exist. Architecture and tests are documented in `OnlyWar_TDD.md §6.6`.

- **Force-level continuation:** after each battle round, each tactical side evaluates its remaining effective Battle Value, recent loss trend, ability to damage the enemy, mission posture, and Aggression. Crossing the Aggression casualty threshold makes withdrawal eligible rather than mandatory; a force that is still winning need not leave merely because it crossed a percentage.
- **Fighting withdrawal:** an organized withdrawal is a multi-squad leapfrog. The squad farthest from the enemy remains Stationary and fires while the other squads Run along a fixed withdrawal axis. It continues covering until it becomes the closest withdrawing squad to the enemy, then the force re-evaluates and assigns the farthest suitable squad as the new cover. A single squad does not Jog backward while firing as a substitute for this maneuver.
- **Pursuit:** a pursuer may Break Off, Follow (Jog and fire within the Jog arc), or Press (Run with no ranged fire). A force that outnumbers the withdrawal and has a speed advantage Presses; a slower withdrawing force cannot escape a faster committed pursuer in open terrain merely by starting early.
...
*Warp lanes.* Each subsector has a designated Imperial warp hub, determined during sector generation by strategic importance, then population and id as deterministic tie-breakers. Only Imperial-controlled worlds are considered when any exist; if a subsector has no Imperial world, the same ranking falls back across all worlds so its derived lane network remains connected. The hub has an established warp lane to every other planet within its subsector. Across subsectors, lanes connect primarily between subsector hubs, with additional cross-subsector lanes possible between high-importance non-hub planets near boundaries. Lanes are derived deterministically during sector generation (and rebuilt on load).
...
- End Turn uses a conditional preflight only when there is meaningful unresolved attention: idle deployable squads, actionable fleets without orders, or unassigned special missions at risk. Special missions are described accurately as having an independent 25% disappearance chance per turn while regional intelligence is at least 1 (and being cleared below 1), not as having a fixed expiry. The player can proceed immediately and may configure global warning preferences; routine turns do not require a redundant confirmation.
...
**What belongs here.** This section records *scope* — what was committed to which release, and what remains. It is deliberately not a description of the work.

- **Behavior** belongs in §4, **architecture** in `OnlyWar_TDD.md`, **detailed formulas and decision records** in `Design/Reference/`, and **per-release change detail** in the GitHub release notes.
...
- ✅ **Battle screen visual overhaul** — replaced the grid-plus-turn-log Battle Review Screen with the four-region replay/report display: view-model layer, summary/analysis service, force hierarchy tree, replay viewport, formation panel and event chronicle, timeline and casualties-by-round table, playback controls, and shared-theme styling. Full spec and known limitations now in **§4.10**.
- ✅ **Strategic Layer Phase 2** — population growth against per-region carrying capacity with a crowding factor; weekly garrison attrition (0.1%) replaced from population growth; OpFor fog of war, recon orders, and special missions (hidden factions concealed, enemy strength graded by intel level, defenses revealed only above an intel threshold); Diversion missions; player-constructable fortifications (Entrenchments, Listening Posts, Anti-Air) via an Intelligence-based Engineering (Fortification) skill; and burrowing/camouflage as ambush tactics (per-species melee and ranged evasion, burrow arrival placement). §4.13, §4.15, §4.20; TDD §6.3 (carrying capacity), §6.4 (diversion effect channels, stealth difficulty), §6.6 (evasion, burrow).
- ✅ **Subsector warp lanes** — per-subsector capital by importance score, lanes from the capital to its subsector, cross-subsector lanes between adjoining capitals, and a spanning tree guaranteeing sector-wide reachability. Derived deterministically from planet positions at generation and rebuilt on load rather than persisted. §4.17; TDD §6.9.
- ✅ **Tyranid Infiltration Units** — Lictor (elite ambusher) and Ravener (fast-attack) species, soldier and squad templates, both eligible for dynamically generated forces. The unused fixed `UnitTemplate` armies for non-player factions were dropped rather than extended. TDD §6.7.
- ✅ **Apothecary Phase 2** — screen rework around the V2 vault/rollup/soldier flow with the Vitruvian wound diagram; weekly natural healing in turn processing; persisted, interactive medical procedures with staff, location, and cost gates; cross-screen recovery display on the Squad Screen; death-record gene-seed recovery; and gene-seed purity as real aggregate domain data. §4.8; TDD §6.6.1.
- ✅ **Supply & Requisition Phase 1 — minimal currency** — the faucet-sink-balance spine that makes Apothecary procedure costs real: a persisted `Army.Requisition` pool seeded at founding, a flat grant on governor-request fulfillment, Apothecary procedures as the sink, and balance/cost display. §4.23. *(Pledges and delivery are Phase 2, §5.3. Founding balance is deliberately generous pending tuning.)*
- ✅ **Game Start Phase 2 — Opening Scenario / "Promised World"** — replaces the bare "name your chapter, pick a seed, sandbox" launch with a framed in-media-res opening: a founding briefing whose promise is attributed to the Sector Lord, a derived governance hierarchy (sector capital and per-subsector seats), the scenario as a deterministic override layer stamped on top of normal generation rather than a generator fork, the sector simulating forward during liberation so requests populate, and win/lapse resolution with a reputation consequence. §4.1, §4.16; full design, locked decisions, and balance anchors in `Design/Reference/OpeningScenario.md`.
- ✅ **Melee Combat Rework — Attack Speed, Multiple Attacks & Weapon Rebalance** — AttackSpeed-driven multiple attacks, weapon speed multipliers, dual wielding, parry on the defender's side of the contested roll, a doubled melee damage scale, and a full `SoldierTemplate.BattleValue` regeneration from an engine-faithful calculator. Balance stance: tabletop is a guide, but base marines are deliberately two-wound soldiers. §4.14 (*Melee Combat*); resolution math, calibration constants, and BV derivation in TDD §6.6.
...
- ✅ **System menu and release-support baseline** — the specialized Save button has been removed in favor of a global System menu with Resume/Save/Load/Return to Title/Quit, dirty-state prompts, global Escape toggle, X-to-close gameplay dialogs, global End Turn warning preferences, and diagnostic export. Title Load is now an explicit chooser; title Options/Quit use the same release-support surfaces. §4.27.
- ✅ **Save confidence** — unlimited named manual slots; visible manual/autosave chooser; initial, protected pre-turn, and three rolling post-turn recovery points; atomic save/metadata replacement; explicit corrupt/incompatible states; recoverability-aware destructive navigation; and persistence for the bounded latest-turn report used by Archive after load. §4.18. *(Save format 7; earlier saves are intentionally incompatible under the alpha save-version policy.)*
...
- **Battle Logic Phase 4B:** Morale, Rout behavior, and post-battle loadout recalculation. *(On-fire damage-over-time/panic was a sub-item here; it is now rejected outright — see below and §4.14.)* *(The organized covered-withdrawal/pursuit/rear-guard system and immediate Burrow disengagement are implemented. Flamers/cone templates were pulled forward to §5.2; grenades and Phase 4A movement tiers shipped in §5.3.)* Sub-item status:
  - ✅ **Morale and Rout.** Per-turn stress scoring, sticky rout state, and its consumption by planning, withdrawal, pursuit, and replay. Architecture in TDD §6.6; §6.2 is closed. Calibration remains the open piece — every tunable lives in code, not the rules DB.
  - ❌ **On-fire damage-over-time / panic — closed as won't-do (2026-08-08).** Not "deferred": the mechanic is rejected on design grounds and should not be reopened without new argument. Full rationale in §4.14 (*Template Weapons — Flamers*), in four parts — the GURPS rule's payoff is action-economy denial and costs a cross-cutting persistent condition (state, saves, replay, planner pricing) the flamer does not need; it assumes clothed humans and forces a per-species burns/cares flag for Tyranids; fire commands no terror premium in a setting of bolt shells, plasma, and lascannons; and morale stress is priced on outcomes rather than weapon flavor, so the flamer already earns its panic through the this-turn casualty fraction its auto-hit cone is unusually good at producing. A fire-specific shock term would both break that rule and double-count the burst.
  - ✅ **Post-battle loadout recalculation — closed as obsolete (2026-08-03).** Resolved by the doctrine work rather than by building what this item described. `BattleSquad.ReallocateEquipment` re-runs full allocation at the start of every battle in a mission, so a fallen carrier's weapon is picked up by the best remaining shooter instead of being lost for the rest of the week. For player squads there is no stored loadout left to re-derive: `LoadoutDoctrineService` resolves planet → chapter → template, and `CharacterLoadoutService` resolves each character's kit, fresh at every allocation. A post-battle mutation pass would now be actively wrong — it could only write into the shared doctrine (hitting every squad of that template) or silently detach the squad from doctrine. Two loose ends were considered and deliberately declined: scaling the pooled loadout to current strength (a mauled squad concentrating its remaining special weapons is the intended reading, not a bug), and honoring element quotas at allocation (elements are purely an equipment-allocation mechanism today — every multi-element template is leader + rank-and-file or an all-character HQ, so there is no distinct sub-unit whose loss should strand its kit).
- ✅ **Wound healing cadence correction.** Natural healing advanced the recovery clock for *every* severity band each week, occupied or not, so a wound stepping down found the band below had already served its dwell time and cascaded straight to Minor in one pass. Measured against the advertised `RecoveryTimeLeft()`, every wound above Moderate healed in roughly a quarter of its stated duration (Unsurvivable: 8 weeks against 28). Corrected so that each band holding wounds advances its own clock concurrently and empty bands do not — which also makes step-down collisions structurally impossible, since a band always empties one week before the band above steps into it. Pinned by `WoundHealingCadenceTests`. **Balance impact is real and unverified in play:** severe wounds now take roughly 3.5× longer to heal than they did in practice, which moves recovery times, Apothecarium readiness, and the value of any future field-medicine system. See §4.12 (*Recovery*) and §6.14.
- ✅ **Leg Wound Realism — delivered without prone combat.** A solid leg hit now staggers rather than reliably felling. Motivated by combat GSW data: even for 40k-tier energies (nothing softer than .45/7.62), only ~45–55% of solid leg hits fracture bone/joint or major vessel and actually prevent movement; the rest slow but don't stop. **Prone fire was cut deliberately, and the item shipped anyway** — graded impairment achieves the stated goal without the squad planner ever learning a new decision. Design record: `Design/Reference/CasualtyRealism.md` (§5 is the confidence assessment behind the cut); architecture in TDD §5.3.
  - ✅ **Motive impairment is graded, not binary.** Rather than raising one threshold, the yes/no "is a motive location crippled?" question was replaced by a **speed multiplier per motive location by wound band**, where immobility is just the product reaching zero. A Critical leg now costs speed instead of ending the fight. Legs cripple at Massive and sever at Mortal — a band apart, so "crippled but not severed" remains reachable, which is what incapacitation is built on.
  - ✅ **Feet never fell a soldier.** A crippled or severed foot floors at a fraction of move speed instead of zero. "Shot in the foot, out of the fight entirely" was the least believable outcome in the old model, and since prone fire was cut, any location that immobilizes removes the soldier outright — which made getting this right more load-bearing, not less.
  - ⬜ **Downed-but-armed enemies keep firing prone** — **moved to §5.7** with the rest of stance. A felled marine is still out of the battle though he holds a bolter: a known fidelity loss, taken deliberately. Its two dependencies (live stance modifiers, prone melee effects) are unchanged and unimplemented, and stance's real payoff is prone *behind cover*, which needs the terrain/LOS system that also sits in §5.7.
- ✅ **Player-Soldier Incapacitation.** *Incapacitated* is now a persistent casualty state separate from both immobility and death, reflecting an Astartes' ability to survive wounds that end his participation in a battle. Three outcomes for player soldiers:
  - **Impaired:** the soldier is slowed but keeps fighting, and recovers normally.
  - **Incapacitated:** alive but removed from combat. Persists into the battle result and the mission debrief as a casualty rather than a kill.
  - **Killed:** follows the existing fallen-brother, gene-seed, and casualty-record rules.
...
- ✅ **Insurrectionist irregular squad types.** The Insurrectionists borrowed a clone of the PDF Infantry Squad — the Imperial faction's own species, soldier templates and weapon set — so a revolt fielded forces indistinguishable from the PDF it rose against. They now have their own species, roles, kit and formations (`Database/RulesMigration_InsurrectionistUnits.sql`), and the PDF clone is retired.
  ...
  - **Rolled squad strength is new machinery, and it is opt-in.** Generation had always built every element at its `MaximumAllowed`; the min/max range was consulted only by the leftover-budget remainder squad. A mob that musters 5–30 needed that to change, but the change could not simply key off `Min < Max`: most ranged elements in the rules data (a Tactical Squad's 4–9 marines, a chapter office's 0–50 specialists) express an establishment squads are filled *to* plus an understrength floor, and rolling them would wreck chapter founding. So `SquadTemplateElement` gained a `RollsStrength` column, set on exactly one element in the whole database. `SquadTemplate.BattleValue` prices a rolling element at its average and `ForceGenerator` now charges the budget for the squad that actually mustered rather than the template's advertised price; both are exact no-ops for every non-rolling element.
  - **Why an HQ, given the faction is a rabble.** `ForceGenerator.GenerateHqEncounter` fields *nothing* for a faction with no HQ-flagged template, so assassination missions against a revolt would have produced an empty opposing force — and decapitating an uprising is the most apt mission the game has. The Firebrand also gives them the only command aura they have. Note the side effect: `GenerateGenericHqSquad` attaches an HQ to any force with budget for it plus three other squads, so the Firebrand appears routinely rather than being a rare encounter. The Imperial PDF still has this hole and is untouched here.
  - **They are not cheaper per man, and that is correct.** Battle values were recomputed with `Helpers/Battles/BattleValueCalculator` against its reference panel: an insurrectionist trooper prices identically to a PDF trooper (5). Measured with everything else held constant, the whole gap between the two is the one MOS point (raw 4.2547 → 4.2421, 0.3%). Two further findings from that sweep, both worth knowing before tuning this faction:
    - **Dropping grenades costs nothing in BV.** A frag adds *exactly zero* to anyone carrying a working primary — offense is identical to five decimal places with and without it. This is the sidearm rule from the grenade-BV pass (§5.3): a soldier fires or throws in a turn, never both, so the calculator takes `max(primary, grenade)` per panel profile rather than summing, and an autogun beats a frag against all four profiles. The grenade only registers when there is no primary to beat (fists-only: raw 1.57 → 2.05).
    - **Armour below roughly 10 is invisible.** Armour 0, 2, 3 and 5 all produce identical durability against the panel — it is a flat damage subtractor that boltguns and devourers simply ignore. Giving the Insurrectionists worse armour would be pure flavour with zero mechanical effect, which is why they keep the PDF's Light Armour.

    The primary weapon, by contrast, is the largest single term: the same trooper with a heavy stubber prices at raw 5.06 against the autogun's 4.24, crossing a BV integer. The faction is therefore distinct in organisation, not in per-man value.
  - **Known approximation:** stored BV is per `SoldierTemplate`, not per carried loadout, so both men in the Weapon Team price at 5 even though the one holding the stubber is worth ~6 — the team prices at 10 rather than ~11. This is the same approximation the Brood Brother Weapon Squad already makes with its autocannon crew. Fixing it properly means a distinct soldier template per crew-served role, which is not scoped here.
  - *(Interim fix retained: a faction with zero squad templates got a **null** `SquadTemplates` map from `GameRulesDataAccess`, which every `ForceGenerator` profile then dereferenced; the generator normalizes null → empty, so a template-less faction degrades to "fields no force" instead of throwing.)*
  - **Godot verification pending:** squad sizes and the new formations have not been seen in the running game.
...
- ✅ **(1) Structured event log substrate + migration + death preservation.** The free-text `List<string>` history is replaced by typed, queryable `SoldierEvent` records carrying date, faction, weapon, magnitude, location, and related-soldier ids, rendering back to the legacy display lines so the history surface is unchanged. All existing write sites emit typed events, persistence moved to a structured table (save compatibility intentionally broken), and fallen brothers are preserved and round-trip through save/load. §4.12; TDD §4.3, §5.3.
...
*Recruitment v1 was pulled forward from this sequence to 0.7.3 (§5.4) and has shipped, so the Promised World win now unlocks something concrete. Chapter Mandates (7) is unaffected — it still follows the Promised World rather than recruitment.*
...
**Live ammunition tracking & reloads.** Finish the ammunition model, which is currently built but disconnected. `RangedWeapon.LoadedAmmo`, `AmmoCapacity`, and `ReloadTime` all exist and persist; the planner already caps a burst at `min(RateOfFire, LoadedAmmo)`, skips empty weapons as attack candidates, and queues a multi-turn `ReloadRangedWeaponAction`. What is missing is consumption: `ShootAction` never decrements `LoadedAmmo`, so only template and blast weapons ever deplete or reload. Ordinary firearms fire indefinitely, their `ReloadTime` is dead data, and the planner's empty-weapon branches are unreachable. Scope:

- **Consume ammunition on ordinary fire.** Decrement in `ShootAction` by the shots actually fired. Open question: the recoil loop can stop a burst early, so decide whether the unfired remainder of a planned burst is spent or retained.
- **Decision-making under scarcity.** Once magazines deplete, the planner should weigh remaining rounds — not merely refuse impossible shots — and squads should avoid reloading in unison. Expect high-`RateOfFire` weapons to get materially worse in play; Battle Value already prices magazine depth through the `sustain` duty cycle, so values should not need regenerating, but seeded battle baselines will diverge.
- **Review shotgun reload times.** Both shotguns carry `ReloadTime` 16 (shell-by-shell tube loading, 8 shells). That is 16 consecutive turns of doing nothing else, and it has never been exercised in play because the reload branch is currently unreachable. Re-tune against the rest of the ladder (grenades 1, small arms 3, missile launcher 4, belt-fed and crew-served 5) before shipping, or the change silently removes shotgun-armed troops from the battle.
- **Decide what Tyranid bio-weapons do.** Deathspitter, Devourer, Spinemaw, and Bio-Plasma are symbiote organisms but carry magazines and `ReloadTime` 5. Harmless today because they never deplete; once consumption is live, Tyranids will stop mid-battle to "reload," which does not fit the faction's fiction. Either exempt bio-weapons from depletion (effectively unlimited, with Battle Value adjusted to match), or reframe the counter as biological exhaustion with a recovery period — a deliberate choice, not a data tweak.
...
### 6.2 Morale — IMPLEMENTED, ARCHITECTURE IN `OnlyWar_TDD.md §6.6`

**Question (answered):** What triggers a morale check and what are its outcomes?

The system resolves all three previously-open questions — triggers, outcomes, and propagation — and folds in three dependencies that turned out to be the same feature: Synapse, rear-guard eligibility, and the withdrawal forecast's objective function. The implemented architecture is summarized in `OnlyWar_TDD.md §6.6`; the durable product decisions are:

- **Trigger — a per-turn stress score, not discrete event checks.** The three agreed triggers (sudden casualty percentage, squad leader death, being outnumbered) become *inputs* to a stateless per-turn score rather than each forcing a roll. Event-triggered checks mean a squad losing 10% a turn for six turns never checks while one losing 25% once does; a recomputed score handles attrition grind naturally and matches how `BattleForceEvaluator` already works. Global context **multiplies** local shock rather than adding to it, so the first casualty when the line is collapsing outweighs the third when the force is winning.
- **Roll per soldier; outcome per squad.** Ego already carries per-soldier variance that nothing else uses; rolling per soldier and aggregating to one squad flag leaves `BattleSquadPlanner` untouched and yields "two men wobbled, the squad held" for free.
- **Outcomes — Steady / Shaken / Routing.** Shaken (will not advance, degraded fire) keeps most checks from being binary catastrophe. Routing reuses the behavior already specified in Battle Continuation (§4.14). **Broken** (post-battle unavailability) and **rally** are both deferred — Broken needs strategic-layer UI, rally needs hysteresis design.
- **Propagation — a stress input, not a contagion roll.** A friendly squad routing within visual range raises stress, read from the turn-start snapshot so iteration order is irrelevant and same-turn avalanches cannot occur. Cascades emerge without a second subsystem.
- **Synapse — a species ability, not a faction flag.** A faction flag cannot express "this gaunt squad is out of range of its Warrior," which is the whole mechanic: killing the synapse creatures collapses the swarm. Covered squads skip the check entirely. This generalises to command auras (and later Ork Mob Rule) as one mechanism with different coefficients, and it is what makes the withdrawal forecast protect a leader — valuing him by what his death costs rather than by his own Battle Value.
- **Rout is predominantly an enemy-side mechanic.** Marines sit high enough on the Ego ladder that a correctly tuned system will almost never break a player squad, and should not. The payoff is enemy forces that stop fighting to the last man.
- **Morale owns a resolve curve and never consumes Ego directly.** Ego already feeds the `leadership` and `ancient` ratings and the Librarius rank gate, so it is not a free calibration knob; the coupling is Space-Marine-only, and non-marine Ego remains free.

**Amends §4.14 Battle Continuation:** the rear-guard objective becomes Battle Value-primary with expected survivors as tie-break (headcount does not survive vehicles), and rear-guard candidates must pass a "will not break while holding" gate satisfied by either high Ego or synapse coverage *that persists for the hold* — coverage at selection time alone would sever when the providers withdraw with the main body, breaking the rear guard mid-hold. The structural protection against sacrificing a safely-escaping squad is unchanged — it was always the exposure gates, not the metric.
...
### 6.14 Graduated Healing Setback on New Wounds (raised by Recovery, §4.12)

**Question:** A new wound to a hit location resets that location's healing clock for *every* wound severity it is carrying. A Negligible graze therefore costs a Massive wound one week short of stepping down its entire accumulated recovery. Should the setback instead be *graduated* — scaled to the severity of the new wound rather than always total?

**Why the current rule is not simply a bug.** Healing progress is tracked per *location*, not per individual wound, and it has to be. Wound accumulation promotes: enough wounds in one severity band become one wound of the band above, which is what lets many small hits eventually disable a location. That promotion destroys wound identity by design — five Major wounds are not literally one Critical wound, they are an abstraction for "this much trauma to one location is functionally critical." So there is no principled answer to *whose* healing progress the promoted wound inherits, and per-wound clocks would not supply one; any inheritance rule (min, max, mean, discard) is arbitrary. Once healing progress is necessarily a location-level property, fresh trauma setting it back is the coherent behavior, and it matches the fiction: a new serious wound genuinely complicates the injuries already present through blood loss, infection load, and immobility.
...
**Status:** deliberately not scheduled. The healing cadence was corrected in 0.7.3 (severe wounds now take roughly 3.5× longer to heal than they did in practice), so the baseline this refinement would tune against has only just changed. Whether the disproportionality actually bites is a play-verification question, not a design one. Revisit if it does.
...
| Combat Pace | The Jog movement tier. The soldier moves at half their MoveSpeed. Shooting is permitted only within 90 degrees of the movement direction, with the full weapon `Bulk` penalty and no aim bonus. Entering Jog resets accumulated aim. |
...
| Subsector Capital | The highest-importance Imperial-controlled planet in a subsector, with population and id as deterministic tie-breakers. If no Imperial world remains, selection falls back across all worlds to preserve warp topology. The capital has established warp lanes to all other planets in the subsector; the subsector's *seat of government* (which cannot use that fallback) is tracked separately (§4.1). |
