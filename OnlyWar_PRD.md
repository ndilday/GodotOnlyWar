...
- ✅ **Grenades / blast templates:** Frag grenades and the Grenade Launcher as blast-template weapons — margin-driven scatter, Strength-scaled throw range, quadratic damage falloff, thrower-included danger-close, third weapon-set ranged slot. Full behavioral spec in §4.14 (*Template Weapons — Grenades*); implementation architecture in TDD §6.6. *(Implemented — `WeaponSet` gained a nullable `GrenadeWeaponId` third ranged slot; two Frag Grenade rows (marine/Throwing and generic/Generic Ranged, TemplateType 3) ship on all Space Marine, PDF, and human-tier Genestealer Cult sets, and the GSC Grenade Launcher converted to launched-blast TemplateType 2. `BlastTemplate` resolves the aim cell and margin-driven scatter (1 cell per point of failure, half-normal by construction) and returns per-victim distances; `BlastAttackAction` pre-resolves once for replay determinism, auto-hits everyone in the circle including the thrower, scales the damage roll by `(1 − d/AreaRadius)²` before armor, and spends its single shot into the 1-turn belt reload. The planner scores nominal-impact blast circles (enemy expected BV removed minus friendly expected BV lost, thrower included) and throws only when that beats the soldier's best conventional action — ties go to the gun; melee-engaged soldiers never throw; empty grenades restock via the normal reload branches on idle turns. Blast volleys queue wounds through the turn resolver and surface as fire-phase volley events in replay summaries. `BattleValueCalculator` (and the offline `Compute-BattleValue.ps1` harness, verified to 6-decimal parity) value blasts via density-scaled auto-hit victims, an average-falloff tunable, reload duty cycle, Strength-scaled reach, and a sidearm max(primary, grenade) rule; regeneration changed no shipped BV row and left the `StrategicCombatRules` anchors untouched. Covered by blast-template, action, planner, reporting, data-access, and BV tests.)*
- ✅ **Detection as search effort (moving stealth difficulty off Garrison):** Stealth difficulty no longer scales with how many enemies *live* in a region, but with how hard they are *looking*. Behavioral spec in §4.11 (*Mission Execution*); formula in TDD §6.4. *(Implemented — `MissionStealthDifficulty` now sums a per-faction WatchScore over every detecting faction in the region: surveillance (`SurveillanceWeight × own regional intel`), an uncapped active-search term over forces on Patrol/Recon orders, and a presence term capped at `AmbientSearchCap`. Motivation: `Garrison` had become an Imperium-only concept among public factions — for a `PopulationIsMilitary` horde `GetDeployedStrength()` resolves to Population, so the old `log10(deployed strength)` term meant "how many creatures live here" and, at `MissionCheck`'s 0.2σ per difficulty point, spanned 1.6σ while every other lever combined spanned ~0.6σ. Mass was not a factor in the check, mass **was** the check. Every term now sits inside `log10(1 + x)`, so all are ≥ 0 by construction: an empty region scores exactly 0 and the recurring `Log(0) = −∞` → margin `+∞` free-success bug is closed off by the shape of the formula rather than by a `Max(1, …)` guard rediscovered at each call site. Patrol and deployed strength are both denominated in **battle value**, the currency `Garrison`/`Population` pools already use. `Region.SelectSpotter` now draws the interceptor weighted by the same WatchScore, so the faction that made the crossing hard and the faction that catches you can no longer disagree.)*
- ✅ **Battle Logic Phase 4A — Movement tiers and sprint/fire tradeoff:** Five squad-level tactical tiers — Stationary, Walk, Jog, Run, and In Melee — now govern movement allowance, legal actions, aim retention, weapon `Bulk` penalties, defensive `CurrentSpeed`, charging, and banked fractional movement. Full behavioral spec in §4.14 (*Movement Tiers — Battle Logic Phase 4A*). *(Implemented. Leg-wound/true-stance integration and Battle Value recalibration remain deliberately deferred.)*
...
- **Player-Soldier Incapacitation (maybe):** Add *incapacitated* as a persistent casualty state separate from both immobility and death, reflecting an Astartes' superhuman ability to survive wounds that end his participation in a battle. This creates three distinct outcomes for player soldiers:
