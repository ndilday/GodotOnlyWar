...
- **Rout:** rout is an involuntary morale state, not a selected withdrawal plan. A routing squad Runs without covering fire or orderly role rotation and remains vulnerable to interception. Exact morale triggers remain open in §6.2; rout reuses the withdrawal/pursuit state and aftermath machinery once those triggers are decided.
...
**Outcomes to define:** reduced combat effectiveness, rout, or broken (squad cannot be assigned orders for a number of turns after the battle ends). Organized covered withdrawal is implemented under Battle Continuation in §4.14, and the shared state/outcome seam for Rout exists. What remains open here is what *triggers* a morale check, which morale outcome results, and how involuntary Routing propagates between squads.
