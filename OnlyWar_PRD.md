...
5. [Release Scoping](#5-release-scoping)
   ...
   - 5.4 [Alpha 0.7.1 — Completed](#54-alpha-071--completed)
   - 5.5 [Alpha 0.7.1 — Stretch Goals](#55-alpha-071--stretch-goals)
   - 5.6 [Alpha 0.8 — Command, Narrative & Continuity](#56-alpha-08--command-narrative--continuity)
   - 5.7 [Alpha 0.8+ — Cross-Faction Simulation (Relationships, Intel, Orks)](#57-alpha-08--cross-faction-simulation-relationships-intel-orks)
...
- **Gated follow-ons (Battle Logic Phase 4, §5.5).** An "on fire" damage-over-time and panic condition is specified in intent but gated on the morale system (§6.2). Cover/terrain interaction with templates is gated on Battle Visuals Phase 3 line of sight.
...
- **Battle Value.** How much the grenade slot adjusts template BV is deliberately deferred until the implementation is locked down; the grenade slot is valued at zero in this pass.
...
This behavioral specification is **implemented for Alpha 0.7.1** (see §5.4) on the **faction-presence model**, deliberately built as a forward-compatible subset of a possible future Pop-based population model (see Open Design Question §6.7).
...
This is a behavioral specification. It is scoped as the **substrate prerequisite** for Orks (see §5.7) and is designed so the Revolt and governor systems can adopt it without rework.
...
This is a behavioral specification. It depends on §4.21 (relationships + intelligence) and is scoped as the dependent Ork line item in §5.7.
...
- *Intelligence / hook* — a narrative lead that surfaces a mission opportunity (feeding the Mission System Expansion §5.5 and the Inquisition §6.5) rather than crediting a resource.
...
This is a behavioral specification. It depends on §4.21 (behavior flags, growth types, intelligence-as-belief) and slots alongside the cross-faction simulation work (§5.7). The current `ScenarioBuilder.StampPromisedWorld` static stamp (§4.1) is the shipped subset this enhances.
...
- ✅ **Strategic Layer Phase 2:**
  ...
  - ✅ Burrowing and camouflage as ambush tactics. *(The 0.7 scope is complete: species evasion and burrow-arrival placement are implemented. Immediate burrow/flight retreat belongs to the broader withdrawal system in Battle Logic Phase 4 rather than this completed 0.7 commitment; see §5.5.)*
...
### 5.6 Alpha 0.8 — Command, Narrative & Continuity
...
### 5.7 Alpha 0.8+ — Cross-Faction Simulation (Relationships, Intel, Orks)
...
**Content:** Dreadnoughts, Chaplains, Psykers, Chaos Troops, Necrons, Tau, Vehicles, Flying Units, Drop Pods, Fortifications, Relics, Poison Weapons, Geneseed Mutation, Power Armor Variants, The Inquisition. *(Orks are now scheduled — see §5.7. When Vehicles arrive, add krak grenades alongside them — thrown single-target anti-armor attacks, not blast templates; deferred from the §4.14 grenade work because they matter little without armored targets.)*
...
| Sector Lord | The governor of the sector capital; the ranking secular Imperial authority in the sector. The character who promises the player's chapter its Chapter World in the Opening Scenario (§5.3). |
