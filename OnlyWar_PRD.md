...
- **Gated follow-ons (Battle Logic Phase 4, §5.4).** An "on fire" damage-over-time and panic condition is specified in intent but gated on the morale system being *built* (§6.2 is resolved as of 2026-07-16; spec in `Design/Active/MoraleAndRout.md`). Fire panic should feed the morale stress function as another shock term rather than becoming a separate escape system. Cover/terrain interaction with templates is gated on Battle Visuals Phase 3 line of sight.
...
**Implementation status (post-0.7).** The organized-withdrawal system is implemented: force-wide BV continuation decisions, leapfrog Cover/Bound movement, Break Off/Follow/Press pursuit, mobility and masked-departure contact breaks, autonomous forecast-gated rear guards, immediate Burrow disengagement, typed outcomes/events, decision-reason logging, and mission/replay/aftermath integration. The state seam for Rout exists; the §6.2 morale design that gated it is resolved as of 2026-07-16, so involuntary routing is now an implementation task rather than an open question — spec in `Design/Active/MoraleAndRout.md`, which also amends the rear-guard objective here to Battle Value-primary and adds a "will not break while holding" gate on rear-guard candidates. Flight will use the same capability seam once flying units exist. Architecture and tests are documented in `Design/Active/WithdrawalAndPursuit.md`.
...
### 6.2 Morale — RESOLVED 2026-07-16, SPEC IN `Design/Active/MoraleAndRout.md`

**Question (answered):** What triggers a morale check and what are its outcomes?

The full behavioral spec, findings, delivery phases, and test plan live in `Design/Active/MoraleAndRout.md`. It resolves all three previously-open questions — triggers, outcomes, and propagation — and folds in three dependencies that turned out to be the same feature: Synapse, rear-guard eligibility, and the withdrawal forecast's objective function. Summary of the decisions:
